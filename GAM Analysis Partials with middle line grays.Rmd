...
# Set Graphics Theme
theme_classic() sets `ggplot()`graphics for no background, no grid lines, etc. 
in a clean format suitable for (some) publications.

The custom theme bundles a number of other aesthetic choices to make graphics
more similar to a preferred design generated in some earlier iteration of the
graphics.
...
ggplot(emms, aes(Pred, response)) +   
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), fill = "grey90") +
  geom_line() +
...
### Save the Plot
```{r}
ggsave(file='figures/density_4.png',
width = 5.04, height = 2.2)
ggsave('figures/density_4.pdf', device = cairo_pdf,
width = 5.04, height = 2.2)
```

## Supplementary Graphic
```{r}
ggplot(turb, aes(Pred, response)) +   
...
### Save Plot
```{r}
ggsave(file='figures/density by turbidity 4.png',
         width = 3.3, height = 2.2)
ggsave('figures/density by turbidity 4.pdf', device = cairo_pdf, 
       width = 3.3, height = 2.2)
```

```{r}
ggsave(file='figures/density by turbidity sq 4.png',
         width = 3.3, height = 3.3)
ggsave('figures/density by turbidity sq 4.pdf', device = cairo_pdf, 
       width = 3.3, height = 3.3)
```
...
###Save the Plot
```{r}
ggsave(file='figures/shannon_4.png',
         width = 5, height = 2.2)
ggsave('figures/shannon_4.pdf', device = cairo_pdf, 
       width = 5, height = 2.2)
```
...
### Save Plot
```{r}
ggsave(file='figures/Acartia_4.png', 
         width = 5.04, height = 2.5)
ggsave('figures/Acartia_4.pdf', device = cairo_pdf, 
       width = 5.04, height = 2.5)
```
...
#### Save Plot
```{r}
ggsave(file='figures/Balanus_4.png',
         width = 5.04, height = 2.5)
ggsave('figures/Balanus_4.pdf', device = cairo_pdf, 
...
### Generate Data
```{r}
p1.emms <- marginal_analysis(dat, Turb, mod, 
                                  .nstops = 25, .logx = TRUE)
```

### Generate Graphic
```{r}
ggplot(p1.emms, aes(Turb, response)) +   
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), fill = "grey80") +
  geom_line() +
  geom_point(data = dat, mapping = aes(x = Turb, y = Density),
             size = 1, color = "grey30") +
  geom_rug(data = dat, mapping = aes(x = Turb, y = NULL)) +
...
### Save Plot
```{r}
ggsave(file='figures/Eurytemora_4.png',
         width = 3.3, height = 2.2)
ggsave('figures/Eurytemora_4.pdf', device = cairo_pdf, 
       width = 3.3, height = 2.2)
```

```{r}
ggsave(file='figures/Eurytemora_sq_4.png',
         width = 3.3, height = 3.3)
ggsave('figures/Eurytemora_sq_4.pdf', device = cairo_pdf, 
       width = 3.3, height = 3.3)
```